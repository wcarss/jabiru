...
let EntityManager = (function () {
  let manager = null,
    ...
    remove_text = function (id) {
      let to_remove = -1,
        i = null,
        ui_context = manager.get('context').get_context("ui"),
        x = 0,
        y = 0,
        width = null,
        height = ui_context.measureText("m").width;

      for (i in texts) {
        if (texts[i].id === id) {
          to_remove = i;
          width = ui_context.measureText(texts[i].text).width;
          x = texts[i].x;
          y = texts[i].y;
        }
      }
      ...
      if (to_remove !== -1) {
        ...
        manager.get('render').clear_context("ui", x, y-height, width, height*1.5);
      }
    },
...
let RenderManager = (function () {
  let manager = null,
    ...
    clear_context = function (id, x, y, width, height) {
      let context = context_manager.get_context(id);

      x = x || 0,
      y = y || 0,
      width = width || context_manager.get_width(id),
      height = height || context_manager.get_height(id);

      context.clearRect(x-1, y, width+2, height);
    },
    clear_all = function () {
      clear_context("main");
      clear_context("ui");
    ...
    text_draw = function (text, context, delta, offset) {
      ...
      context.clearRect(x, y-last_height, last_width, last_height*1.5);
